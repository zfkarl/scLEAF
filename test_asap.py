...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", type=str, default= '/data2/zeyu/zeyu1/scLEAF-v2/results/cite-asap/1_False_500_0.00015_asap/lightning_logs/checkpoints/last.ckpt')
    # Dataset
    ...
    atac_test_loader = torch.utils.data.DataLoader(atac_data.test, batch_size = 512, drop_last=False,num_workers=4)
    
    for batch in atac_test_loader:
        sample, text , gene,label = batch
        sample, label = sample.cuda(), label.cuda()
        with torch.no_grad():
            cell_emb = atac_cls_model._get_cell_features(sample)
            atac_logit = atac_cls_model.classification_head(cell_emb).cpu().numpy()
            atac_pred = np.argmax(atac_logit, axis=1)

        atac_logits.append(atac_logit)
        atac_preds.append(atac_pred)
        atac_labels.append(label.cpu().numpy())
        atac_embs.append(cell_emb.cpu().numpy())

    atac_logits = np.concatenate(atac_logits, axis=0).reshape(-1,atac_classes)
    atac_preds = np.concatenate(atac_preds, axis=0).flatten()
    atac_labels = np.concatenate(atac_labels, axis=0).flatten()
    atac_embs = np.concatenate(atac_embs, axis=0)
    
    print('atac_embs shape: ',atac_embs.shape)
    
    idx2label = {0: 'B', 1: 'DC', 2: 'Effector CD4+ T', 3: 'Effector CD8+ T', 4: 'Monocytes', 5: 'NK', 6: 'Naive CD4+ T', 7: 'Naive CD8+ T', 8: 'unknown'}

    celltypes = [idx2label[label] for label in atac_labels]
    
    data_dict = {
    'atac_predictions': atac_preds,
    'atac_labels': atac_labels,
    'atac_embeddings': atac_embs,
    'atac_logits': atac_logits,
    'celltypes': np.array(celltypes)
}

np.savez('/data2/zeyu/zeyu1/scLEAF-baselines/visualization/umap/asap_test_scleaf2.npz', **data_dict)